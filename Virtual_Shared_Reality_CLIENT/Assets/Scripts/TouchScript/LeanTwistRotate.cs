...
namespace Lean.Touch
{
	...
	public class LeanTwistRotate : MonoBehaviour
	{
		...
		protected virtual void Update()
		{
			...
			{
				if (UpPos.TryMove(this.gameObject))
				{
					if (Relative == true)
					{
			...
			if (transform.localRotation != oldRotation)
			{
			
				m_hit = UpPos.CheckCollision(this.gameObject);
		...
		protected virtual void Rotate(float twistDegrees)
		{
			...
			if (camera != null)
			{
				...
				newRotation = Quaternion.AngleAxis(twistDegrees, axis);
				transform.rotation *= newRotation;
				/* should I add something here ? */
				TwistPerformed?.Invoke();
			}
			else
			{
				Debug.LogError("Failed to find camera. " +
					"Either tag your cameras MainCamera, or set one in this component.", this);
			}
		}
	}
}