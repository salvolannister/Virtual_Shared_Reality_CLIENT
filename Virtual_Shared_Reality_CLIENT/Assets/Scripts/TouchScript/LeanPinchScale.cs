	...
	public class LeanPinchScale : MonoBehaviour
	{
		...
		protected virtual void Update()
		{
			...
			if (pinchScale != 1.0f)
			{
				...
				if (UpPos.TryMove(this.gameObject) || ClientManager.Instance.TUTORIAL)
				{
					if (Relative == true)
					{
				...
				/* why it works ? I should see the changes but I dont*/
		...
		protected virtual void Translate(float pinchScale, Vector2 screenCenter)
		{
			...
			if (camera != null)
			{
				...
				// Convert back to world space
				//m_hit = UpPos.CheckCollision(newposition, this.gameObject);
				//if (!m_hit)
